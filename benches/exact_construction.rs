...
fn concat_builder(parts: &[&str]) -> LeanStr {
    let capacity = parts.iter().map(|part| part.len()).sum();
    let mut value = LeanString::with_capacity(capacity);
    for part in parts {
        value.push_str(part);
    }
    value.freeze()
...
fn join_builder(parts: &[&str], separator: &str) -> LeanStr {
...
fn concat(c: &mut Criterion) {
    ...
    for case in &CONCAT_CASES {
        ...
        group.bench_function(BenchmarkId::new("lean_str", case.name), |b| {
            b.iter(|| black_box(LeanStr::concat(black_box(case.parts))))
        });
...
fn join(c: &mut Criterion) {
    ...
    for case in &JOIN_CASES {
        ...
        group.throughput(Throughput::Bytes(bytes as u64));
        group.bench_function(BenchmarkId::new("lean_str", case.name), |b| {
            b.iter(|| black_box(LeanStr::join(black_box(case.parts), separator)))
        });
        group.bench_function(BenchmarkId::new("builder_freeze", case.name), |b| {
